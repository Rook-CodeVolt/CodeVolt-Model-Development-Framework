#!/usr/bin/env python3
"""ADR-0018 bounded DPO cycle runner (confabulated-recipe-refusal axis, per
ADR-0017's preference-pair package).

This is a mechanical adaptation of the proven ADR-0016 runner
(examples/pilot-metatrainer-v2/run_bounded_cycle_adr0016.py) to the first
governed run of ``DPOTrainerAdapter`` (``src/codevolt_mdf/dpo_adapter.py``,
PR #98) against the ADR-0017 preference-pair package
(``examples/pilot-metatrainer-v3-dpo/``, PR #98), per the exact
hyperparameter/gate configuration recorded in
``docs/decisions/ADR-0018-dpo-execution-config.md`` (PR #99). That document
states explicitly, in its own "What this document does and does not
authorize" section, that it adds no runner script and that writing one
implementing exactly its configuration is a required, separate follow-up
PR -- this file is that follow-up. It reuses ADR-0016's runner structure,
gate-verification logic, and dry-run/--execute split unchanged in design;
the things that differ are the trainer adapter (DPOTrainerAdapter instead
of TRLTrainerAdapter), the dataset (the ADR-0017 22-record preference-pair
package instead of SFT messages), a second (reference) model identity to
verify, the DPO-specific hyperparameters (beta/reference_free/max_length/
max_prompt_length), the higher memory budget (max_memory_mb=20480, per
ADR-0018 item 6), and a fresh run id/seed/approval namespace/reviewed host
paths, per this project's "fresh identity per new proposal" convention.

Like every prior runner, this file's own writing does not authorize
execution: writing it is the follow-up ADR-0018 itself explicitly
authorizes ("Owner authorization" on the commissioning internal tracking record);
running it (--execute) still requires all three fresh gate signatures
(security, dataset-rights, owner) bound to this exact file's own commit SHA,
verified with the same ssh-keygen -Y mechanism every prior runner uses. No
gate is signed by this file or by writing it.

The default/check mode performs static, hash, dependency, and
TrainerAdapter.prepare() validation only. It never calls the trainer or
evaluator. Live baseline/training/post-evaluation requires ``--execute``
plus a review-gate JSON created after the exact commit receives the
approvals named in ADR-0018.

This script records measurements only. It never promotes, publishes,
deploys, or starts another cycle.
"""
...
# ADR-0018 item 1: policy model and reference model are the same immutable
# checkpoint (full-parameter DPO starting fresh from the base, mirroring
# every prior real cycle). model_path and reference_model_path are the
# identical filesystem path; DPOTrainerAdapter.prepare() still independently
# content-hashes and verifies the reference-model path exactly as the
# policy model, per its own contract (it does not special-case identical
# paths) -- this runner passes the identical path/hash for both rather than
# asserting the adapter special-cases sameness.
REFERENCE_MODEL_PATH = MODEL_PATH
EXPECTED_REFERENCE_MODEL_HASH = EXPECTED_MODEL_HASH
...
# The ADR-0017 DPO preference-pair package lives in a sibling directory to
# this runner (examples/pilot-metatrainer-v3-dpo/), not alongside this
# script -- same file-placement pattern as ADR-0016's SFT corpus. The
# meta_trainer/capability_retention/safety held-out evaluation suites are
# unchanged from ADR-0016 (this document proposes no evaluator or held-out
# suite change; see ADR-0018 item 5), so those paths/hashes are carried
# forward unmodified.
DATASET_DIR = REPO_ROOT / "examples/pilot-metatrainer-v3-dpo"
TRAIN_PATH = DATASET_DIR / "preference_pairs.jsonl"
V3_DIR = REPO_ROOT / "examples/pilot-metatrainer-v3"
HELD_OUT_PATH = V3_DIR / "held_out.json"
REGISTRY_INPUT_PATH = V3_DIR / "held_out_exclusion_registry.json"
...
# Real sha256 hashes independently recomputed against the merged main-branch
# content during this runner's own drafting (shasum -a 256), identical to
# the values ADR-0018 itself independently re-verified and recorded (item
# 2's per-file table). Only preference_pairs.jsonl's hash is load-bearing
# for prepare()'s dataset_hash check; the other four are support files,
...
EVALUATION_SUITES = {
    "meta_trainer": {
        "path": HELD_OUT_PATH,
        "file_hash": EXPECTED_HELD_OUT_SUITE_HASHES["held_out.json"],
        "package_id": "pilot-metatrainer-v3-heldout",
        "count": 48,
        "task_type": "exact_match",
        "registry_path": REGISTRY_INPUT_PATH,
        "registry_hash": EXPECTED_HELD_OUT_SUITE_HASHES["held_out_exclusion_registry.json"],
    },
...
# cycle's identity" convention -- matches ADR-0018 item 3's own table value.
SEED = 20260923
# ADR-0018 item 3: one full epoch over the 22-record preference-pair train
# split, per_device_train_batch_size=1, no gradient accumulation -- the same
# "one step per example, one epoch per corpus size" invariant ADR-0014
# established and ADR-0015/0016 each re-confirmed, applied here at DPO's
# much smaller dataset size. No evidence motivates multi-epoch training on
# a preference-pair dataset this small.
MAX_STEPS = 22
# ADR-0018 item 3: DPOTrainerAdapter's own documented default (TRL's own
# DPOConfig library default is 1e-6; the adapter's module docstring states
# 5e-7 explicitly). Kept at the adapter's own stated default: there is no
# real prior DPO run on this project yet to motivate a deviation.
LEARNING_RATE = 5e-7
BATCH_SIZE = 1
# ADR-0018 item 3: 50% of max_steps (one checkpoint at the run's midpoint),
# the same fraction ADR-0014/0015/0016 each used: round(0.5 * 22) = 11.
SAVE_STEPS = 11
SAVE_TOTAL_LIMIT = 2
MAX_NEW_TOKENS = 160
# ADR-0018 item 3: beta=0.3, the paper's own tested upper-middle value, not
# TRL's lower-end library default of 0.1 -- see ADR-0018's own justification
# table for the full reasoning; this runner only implements the value, it
# does not re-derive it.
BETA = 0.3
REFERENCE_FREE = False
MAX_PROMPT_LENGTH = 512
MAX_LENGTH = 1024
RUN_ID = "adr0018-metatrainer-dpo-20260923"
HOST_CONTAINMENT_SCOPE = "complete-cycle-host-containment-v1"
HOST_CONTAINMENT_ENV = "CODEVOLT_ADR0018_HOST_CONTAINMENT"
# ADR-0018 item 6: fresh reviewed root, mirroring ADR-0013/0014/0015/0016's
# own reviewed-path pattern (a fresh, never-reused root per cycle).
APPROVED_ROOT = Path("./local-evidence/adr0018")
APPROVED_EVIDENCE_ROOT = APPROVED_ROOT / "evidence"
APPROVED_REVIEW_GATE_PATH = APPROVED_EVIDENCE_ROOT / "adr0018-review-gate.json"
APPROVED_SCRATCH_ROOT = APPROVED_ROOT / "scratch"
APPROVED_VALIDATION_SCRATCH = APPROVED_SCRATCH_ROOT / "adr0018-check"
...
def _host_containment_profile(scratch_root: Path) -> str:
    """Build the macOS Seatbelt profile for the complete ADR-0018 cycle.

    Identical mechanism to ADR-0016's runner: allows ordinary reads and
    process execution (Python, MPS, the pinned model cache, and installed
    packages all live outside the run directory), denies all network
    operations and all filesystem writes except beneath the exact reviewed
    scratch root plus writes to the kernel-discarded ``/dev/null`` device.
    Seatbelt restrictions are inherited by subprocesses and native
    extensions.
    """
...
def _verify_host_containment(scratch_root: Path) -> dict[str, Any]:
    ...
    canary = scratch_root.parent / ".adr0018-host-containment-canary"
    if canary.exists():
...
def _run_under_host_containment(scratch_root: Path) -> int:
    ...
    if sys.platform != "darwin" or not sandbox_exec.is_file():
        raise SystemExit("ADR-0018 execution requires macOS /usr/bin/sandbox-exec")
...
def _evaluation_suites(
    ...
    # The ADR-0017 preference-pair package uses pair_id, not example_id (its
    # own prompt/chosen/rejected shape, distinct from the messages-list SFT
    # corpus shape ADR-0016 registered by example_id). Registering by
    # pair_id keeps the same "record every training id so contamination
    # checks below can prove no overlap with any held-out suite" invariant,
    # adapted to this package's own id field.
    train_ids = [item["pair_id"] for item in train_records]
    registry.register_package_train("pilot-metatrainer-v3-dpo-train", train_ids)
...
def _build_inputs(dataset_licence: str) -> TrainingInputs:
    return TrainingInputs.create(
        ...
        dataset_version=f"pilot-metatrainer-v3-dpo@{EXPECTED_FILE_HASHES['preference_pairs.jsonl']}",
        dataset_hash=EXPECTED_FILE_HASHES["preference_pairs.jsonl"],
        seed=SEED,
        training_params={
...
def _budget(scratch_root: Path) -> ResourceBudget:
    return ResourceBudget(
        ...
        # ADR-0018 item 6: raised from ADR-0016's 16384 to 20480 (+4096MB,
        # +25%) -- a fresh review, not a copy of ADR-0015/0016's numbers.
        # DPO holds two full 135M-parameter model copies resident
        # simultaneously (policy + frozen reference), where every prior
        # real cycle held exactly one; ADR-0016's real measured
        # single-model peak was 11,604MB under the 16384MB ceiling. This
        # is a conservative, explicitly-reasoned estimate pending the real
        # first execution's own measurement, not a guarantee -- see
        # ADR-0018 item 6 for the full reasoning; this runner only
...
def validate_plan(scratch_root: Path) -> dict[str, Any]:
    ...
    if _hash_path_identity(REFERENCE_MODEL_PATH) != EXPECTED_REFERENCE_MODEL_HASH:
        raise SystemExit("reference model snapshot content hash mismatch")
    ...
    if len(train_records) != 22:
        raise SystemExit("expected exactly 22 preference-pair train records")
    if len(held_out_records) != 48:
        raise SystemExit("expected exactly 48 held-out records")
    # The ADR-0017 preference-pair package proposes no held-out preference
    # pairs of its own (every record is split == "train", re-verified by
    # ADR-0018 item 2 and this package's own validate_dataset.py). The only
    # held-out overlap check that applies here is against the unchanged
    # meta_trainer/capability_retention/safety suites below, performed via
    # the pair_id-vs-example_id disjointness check in _evaluation_suites.
    if not all(item.get("split") == "train" for item in train_records):
        raise SystemExit("ADR-0017 preference-pair package must be all-train-split")
    ...
    inputs.validate()
    adapter = DPOTrainerAdapter(work_dir=scratch_root / "trainer_work")
...
def _load_gate(path: Path) -> dict[str, Any]:
    ...
    if gate["dataset_hash"] != EXPECTED_FILE_HASHES["preference_pairs.jsonl"]:
