#!/usr/bin/env python3
"""ADR-0014 corrected bounded pretrained meta-trainer cycle runner.

This is a fresh, independently reviewed bounded cycle superseding ADR-0013's
locked hyperparameters. ADR-0013's first (and only) real bounded run
(commit 13b316e, run_id adr0013-metatrainer-sft-20260920) completed cleanly
(training.status=accepted, real eval ran) but hit ADR-0013's own
"stop and reject, do not retry under this ADR" trigger: zero rubric/exact-match
improvement on the target task plus capability and safety regressions, with raw
outputs showing classic overfitting/degeneration (repetitive token loops). See
docs/decisions/0014-corrected-bounded-metatrainer-cycle.md for the full evidence
basis and rationale. ADR-0013's own text requires a new issue/ADR with a fresh
bounded configuration for any continuation -- this script and its governing ADR
are that fresh proposal, not an unrecorded extension of ADR-0013.

It reuses ADR-0013's corpus, evaluator, contamination machinery, resource budget,
and host-containment/approval-verification mechanism completely unchanged. Only
the training hyperparameters (max_steps, learning_rate, save_steps), the run id,
the approval namespace, and the reviewed host paths are new, to keep this cycle
...
JSON created after the exact commit receives the approvals named in ADR-0014.
...
APPROVAL_ALLOWED_SIGNERS_PATH = HERE / "approval_allowed_signers_adr0014"
...
EXPECTED_FILE_HASHES = {
    ...
    "SOURCE_MAP.md": "8d199b33983e19a41d182c6e61f36b00f3ea362bd708147f6952755c33f08cb4",
    "DATASET_CARD.md": "79ce57e6294a697ab94aaea168636f0a4d2d664f95aad4c94b3890866ac8f5c5",
...
# batch_size=1 over the locked 40-example train split) to 40 (1 epoch). The
# real ADR-0013 run (adr0013-metatrainer-sft-20260920, commit 13b316e) trained
# 3 epochs and produced zero-or-negative results on all three held-out suites
# (meta_trainer 0.0%->0.0%, capability_retention 30%->0.0%, safety
# 46.7%->33.3%) with raw candidate outputs showing degenerate repetition loops
# ("46-46-46-...", "PEPFET, a variant of PEPFET, ..."), the textbook signature
# of overfitting a full-parameter SFT run on a very small corpus. One epoch is
# the smallest unit that still completes a full pass over the locked 40-example
# split; it is the minimum change that directly targets the observed failure
# mode without touching the dataset, evaluator, or gates. See ADR-0014 for the
# full evidence basis.
MAX_STEPS = 40
# ADR-0014 correction: LEARNING_RATE halved from ADR-0013's 1e-5 to 5e-6.
# ADR-0013 already selected 1e-5 as "the low end of the cited full-SFT starting
# region"; it was not itself excessive relative to common full-SFT practice.
# However, at 1 epoch instead of 3, this cycle only gets one bounded pass over
# 40 examples, so overfitting must be controlled through the epoch reduction
# above, not through further LR reduction, or the run risks producing too small
# a training signal to observe any candidate/baseline difference at all. LR is
# still lowered modestly (not left unchanged) because full-parameter SFT (not
# LoRA) on a 135M model updates every parameter every step, and the two
# corrective levers this ADR pulls -- fewer steps and a smaller per-step update
# -- are complementary defence-in-depth against re-hitting the same
# degeneration pattern, not redundant. Halving (rather than a steeper cut) is
# chosen to keep the training signal a live, comparable second measurement
# point in the same experimental family as ADR-0013, not a barely-perturbing
# no-op run.
LEARNING_RATE = 5e-6
BATCH_SIZE = 1
SAVE_STEPS = 20
SAVE_TOTAL_LIMIT = 2
MAX_NEW_TOKENS = 160
RUN_ID = "adr0014-metatrainer-sft-20260922"
HOST_CONTAINMENT_SCOPE = "complete-cycle-host-containment-v1"
HOST_CONTAINMENT_ENV = "CODEVOLT_ADR0014_HOST_CONTAINMENT"
APPROVED_ROOT = Path("./local-evidence/adr0014")
APPROVED_EVIDENCE_ROOT = APPROVED_ROOT / "evidence"
APPROVED_REVIEW_GATE_PATH = APPROVED_EVIDENCE_ROOT / "adr0014-review-gate.json"
APPROVED_SCRATCH_ROOT = APPROVED_ROOT / "scratch"
APPROVED_VALIDATION_SCRATCH = APPROVED_SCRATCH_ROOT / "adr0014-check"
...
def _host_containment_profile(scratch_root: Path) -> str:
    """Build the macOS Seatbelt profile for the complete ADR-0014 cycle.
...
def _verify_host_containment(scratch_root: Path) -> dict[str, Any]:
    ...
    canary = scratch_root.parent / ".adr0014-host-containment-canary"
    if canary.exists():
...
def _run_under_host_containment(scratch_root: Path) -> int:
    ...
    if sys.platform != "darwin" or not sandbox_exec.is_file():
        raise SystemExit("ADR-0014 execution requires macOS /usr/bin/sandbox-exec")
