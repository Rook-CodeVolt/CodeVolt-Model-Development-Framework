...
add("zero_score_diagnostic_ladder",
    ...
    ["[1] TRL v0.21 SFT Trainer — Computing the loss; Train on assistant messages only", "research synthesis, SS4.2"])
add("zero_score_diagnostic_ladder",
    ...
    ["[1] TRL v0.21 SFT Trainer — logged optimizer steps", "research synthesis, SS4.2"])
add("zero_score_diagnostic_ladder",
    ...
    ["[1] TRL v0.21 SFT Trainer — logged loss, token accuracy, learning rate, and gradient norm", "[2] Axolotl Training Stability & Debugging — plateau and instability diagnostics", "research synthesis, SS4.2"])
add("zero_score_diagnostic_ladder",
...
add("zero_score_diagnostic_ladder",
    ...
    ["research synthesis, SS4.2", "research synthesis, SS5"])
...
add("sft_objective_masking_format",
    ...
    ["[1] TRL v0.21 SFT Trainer — completion-only loss", "research synthesis, SS1.1"])
add("sft_objective_masking_format",
...
add("sft_objective_masking_format",
    ...
    ["[1] TRL v0.21 SFT Trainer — chat templates and EOS alignment", "research synthesis, SS1.1"])
add("sft_objective_masking_format",
    ...
    ["[1] TRL v0.21 SFT Trainer — completion/assistant-only loss", "research synthesis, SS1.1"])
add("sft_objective_masking_format",
...
add("sft_objective_masking_format",
    ...
    ["[1] TRL v0.21 SFT Trainer — loss masking and chat templates", "[2] Axolotl Training Stability & Debugging — inspect tokenized samples", "research synthesis, SS1.1"])

# TRAIN FAMILY 3: qualified hyperparameter sweeps (8)
add("qualified_hyperparameter_sweeps",
    "Give a defensible full-SFT learning-rate region from the evidence pack.",
    ...
    ["[2] Axolotl Training Stability & Debugging — full fine-tuning 1e-5 to 5e-5", "[3] Hugging Face Transformers Fine-tuning — tutorial uses 2e-5", "[11] MiniMind train_full_sft.py — default 1e-5", "research synthesis, SS1.2"])
add("qualified_hyperparameter_sweeps",
    "Give a defensible LoRA or QLoRA learning-rate region from the evidence pack.",
    ...
    ["[1] TRL v0.21 SFT Trainer — Train adapters with PEFT", "[2] Axolotl Training Stability & Debugging — LoRA 1e-4 to 3e-4", "[4] Unsloth LoRA Hyperparameters Guide — 2e-4 starting point", "research synthesis, SS1.2"])
add("qualified_hyperparameter_sweeps",
    ...
    ["[3] Hugging Face Transformers Fine-tuning — tutorial uses 3 epochs", "[4] Unsloth LoRA Hyperparameters Guide — 1 to 3 epochs", "[11] MiniMind train_full_sft.py — default 2 epochs", "research synthesis, SS1.2"])
add("qualified_hyperparameter_sweeps",
...
add("qualified_hyperparameter_sweeps",
    ...
    ["research synthesis, SS5"])
add("qualified_hyperparameter_sweeps",
    "What full-SFT sweep anchors does the research synthesis recommend for the baseline?",
    ...
    ["research synthesis, SS5", "[2] Axolotl Training Stability & Debugging — full fine-tuning 1e-5 to 5e-5", "[3] Hugging Face Transformers Fine-tuning — tutorial uses 2e-5 and 3 epochs", "[4] Unsloth LoRA Hyperparameters Guide — 1 to 3 epochs", "[11] MiniMind train_full_sft.py — default 1e-5 and 2 epochs"])
add("qualified_hyperparameter_sweeps",
    ...
    ["[1] TRL v0.21 SFT Trainer", "[2] Axolotl Training Stability & Debugging", "[4] Unsloth LoRA Hyperparameters Guide", "[23] Axolotl Quickstart", "research synthesis, SS1.2"])
...
add("peft_lora_qlora_decisions",
    ...
    ["[6] LoRA paper — frozen pretrained weights and low-rank adaptation", "research synthesis, SS2"])
add("peft_lora_qlora_decisions",
...
add("peft_lora_qlora_decisions",
    ...
    ["[6] LoRA paper — GPT-3 175B experimental setting", "research synthesis, SS2"])
add("peft_lora_qlora_decisions",
...
add("peft_lora_qlora_decisions",
    ...
    ["[7] QLoRA paper — 65B model on one 48GB GPU", "research synthesis, SS2"])
add("peft_lora_qlora_decisions",
    "When is full SFT reasonable to benchmark?",
    "Benchmark it when the model fits comfortably, the desired shift is broad, and adapters may lack capacity. This is qualified decision guidance, not a direct claim that full SFT always wins on small models.",
    ["research synthesis, SS2"])
add("peft_lora_qlora_decisions",
    ...
    ["research synthesis, SS2", "[6] LoRA paper — frozen base and low-rank adapters"])
add("peft_lora_qlora_decisions",
    "When should QLoRA be chosen according to this guidance?",
    ...
    ["research synthesis, SS2", "[7] QLoRA paper — quantized frozen base with LoRA adapters"])
...
add("contamination_split_controls",
    ...
    ["research synthesis, SS3.2", "[16] Google ML Crash Course — separate train/validation/test data and avoid duplicates"])
add("contamination_split_controls",
...
add("contamination_split_controls",
    ...
    "They miss normalized variants and paraphrases. The research synthesis recommends normalized-text, token-n-gram, and semantic-similarity scans plus an overlap ledger; that operational prescription is its own synthesis, not a claim that Google specifies those mechanisms.",
    ["research synthesis, SS3.2", "[16] Google ML Crash Course — exact duplicate separation"])
add("contamination_split_controls",
...
add("contamination_split_controls",
    ...
    ["[16] Google ML Crash Course — repeated use wears out validation/test data", "research synthesis, SS3.2"])
add("contamination_split_controls",
...
add("limited_data_evaluation",
    ...
    ["research synthesis, SS4.1", "[1] TRL v0.21 SFT Trainer — metrics and masking"])
add("limited_data_evaluation",
    ...
    ["research synthesis, SS4.1", "[1] TRL v0.21 SFT Trainer — loss and token-accuracy metrics"])
add("limited_data_evaluation",
    ...
    ["[20] OpenAI Evaluation Best Practices — task-specific evaluation and constrained judgments", "research synthesis, SS4.1"])
add("limited_data_evaluation",
    ...
    ["[20] OpenAI Evaluation Best Practices", "research synthesis, SS4.1"])
add("limited_data_evaluation",
    "What challenge slices does the research synthesis recommend for the bounded trainer model?",
    "Include unseen phrasings, contradictory premises, insufficient-evidence cases, wrong-tool choices, LR/epoch troubleshooting, contamination questions, and out-of-scope requests. This list is drawn from the research synthesis.",
    ["research synthesis, SS4.1"])
add("limited_data_evaluation",
...
add("limited_data_evaluation",
    ...
    ["[21] tinyBenchmarks paper — benchmark-specific 100-item subsets using IRT and previous-model data", "research synthesis, SS4.1"])
add("limited_data_evaluation",
    ...
    ["[22] lm-evaluation-harness v0.4.13 release notes", "research synthesis, SS4.1"])
...
add("synthetic_data_tradeoffs",
    ...
    ["[18] Self-Instruct paper — Sections 1-2", "research synthesis, SS3.3"])
add("synthetic_data_tradeoffs",
    ...
    ["[19] Nature model-collapse paper — abstract and Definition 2.1", "research synthesis, SS3.3"])
add("synthetic_data_tradeoffs",
...
add("synthetic_data_tradeoffs",
    ...
    ["research synthesis, SS3.3"])
add("synthetic_data_tradeoffs",
    "Name the main synthetic-data risks the research synthesis identifies.",
    ...
    ["research synthesis, SS3.3", "[19] Nature model-collapse paper — recursive-training risk"])
...
add("catastrophic_forgetting_retention",
    ...
    ["[6] LoRA paper — frozen base weights and low-rank updates", "research synthesis, SS1.3", "research synthesis, SS2 — reversible experiments"])
add("catastrophic_forgetting_retention",
    ...
    ["[6] LoRA paper — frozen base and learned adapters", "research synthesis, SS1.3"])
add("catastrophic_forgetting_retention",
    "What is the purpose of a frozen base-regression suite?",
    "It supplies a stable reference for pre-existing instruction following, basic language behavior, safety boundaries, and prior capabilities. The research synthesis recommends it as a control; its exact composition should match deployment scope.",
    ["research synthesis, SS1.3", "research synthesis, SS4.1 — capability-retention suite composition"])
add("catastrophic_forgetting_retention",
    ...
    ["research synthesis, SS1.3", "research synthesis, SS5"])
add("catastrophic_forgetting_retention",
    "What is an evidence-qualified promotion rule for a tuned checkpoint?",
    "Require a predeclared target-task gain, no material base-regression failure, and a paired improvement larger than observed run or evaluation variability. This is the research synthesis's recommended gate, not a universal external standard.",
    ["research synthesis, SS5"])
