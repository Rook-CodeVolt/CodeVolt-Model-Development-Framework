...
- Scope: scores held-out examples one at a time via a minimal exact-match/containment check against the model's decoded continuation. Deliberately minimal — not a general-purpose evaluation harness. Goes through the same `HeldOutExclusionRegistry` bidirectional contamination check as every other conforming evaluator adapter, and has exactly one public method (`score_example`) with no authority to accept, reject, or promote a candidate.
- Tests: `tests/test_hf_local_evaluator_adapter.py`, including an AST-import test proving this module has no import relationship with `trl_adapter.py`/`trainer_contract.py` (trainer/evaluator role separation).
