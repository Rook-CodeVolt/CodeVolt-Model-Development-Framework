...
- [partially done] Evaluation suites covering capability, safety, and regression — a real (non-fake) `EvaluatorAdapterV1` now exists (`src/codevolt_mdf/hf_local_evaluator_adapter.py`), scoring held-out examples via local HF inference with bidirectional contamination checking against the training data. It covers three task-type modes (exact-match, multiple-choice log-likelihood, format-conformance — ADR-0007, issue #24 WP-A), each independently contract-tested. Regression tracking across candidate revisions is now real too (`src/codevolt_mdf/regression_check.py`, ADR-0009, issue #24 WP-C — measurement-only comparison against baseline and previous-accepted candidate for previously-passing examples, 16 tests including two real-adapter integration tests against the pinned checkpoint). Safety/red-team probing (WP-B) — named in this line's original scope — is implemented but not yet merged (PR pending Maya-CodeVolt review); see [docs/PROGRESSION.md](PROGRESSION.md) for the full priority framing and issue #24 for status.
