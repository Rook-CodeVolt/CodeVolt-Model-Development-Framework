...
- Next queued decision: one pretrained meta-trainer SFT cycle is proposed (not authorized) at [ADR-0013](decisions/0013-bounded-pretrained-metatrainer-cycle.md), tracked by issue #79. It uses the independently audited 40/20 corpus v2, requires baseline-before-training and identical post-evaluation through the corrected chat-template-aware evaluator, and rejects a repeat of ADR-0011's random-initialization recipe. Live execution remains blocked on independent dataset rights/privacy admission, Maya's exact-candidate security review, and owner confirmation for this one run; no automatic retry, promotion, deployment, or next cycle is authorized.
